...
export const PalGroupDetailsSheet: React.FC<PalGroupDetailsSheetProps> = ({
  ...
  // 1. Build uniform array of all slots (joined + empty)
  type SlotItem =
    | { type: 'member'; member: PalGroupMember; slotIdx: number }
    | { type: 'empty'; slotIdx: number };

  const allSlots: SlotItem[] = [
    ...joinedMembers.map((m, idx) => ({ type: 'member' as const, member: m, slotIdx: idx })),
    ...Array.from({ length: emptySlotsCount }).map((_, idx) => ({
      type: 'empty' as const,
      slotIdx: joinedMembers.length + idx,
    })),
  ];

  // Base 3-box height criteria (standard single card ~232dp, 3-card total ~700dp)
  const base3CardHeight = Math.round(cardWidth * (9.5 / 16) + 15);
  const base3TotalHeight = base3CardHeight * 3 + 4;
  ...
  let rowConfigs: RowConfig[] = [];
  let topScrollPadding = Math.max(insets.top - 1, 7) + 55 + 19;
  let rowGap = 2;

  if (maxSlots === 2) {
    // 2 member view: 2 full-width boxes at upper and middle half of screen (one center-up, one center-down)
    const h = base3CardHeight;
    const availableHeight = screenHeight - (Math.max(insets.top - 1, 7) + 55) - (Math.max(insets.bottom, 24) + 32);
    const total2CardsHeight = h * 2 + 2;
    const extraPad = Math.max(0, (availableHeight - total2CardsHeight) / 2);
    topScrollPadding = Math.max(insets.top - 1, 7) + 55 + extraPad;
    rowGap = 2;
    rowConfigs = [
      { items: allSlots.slice(0, 1), layoutType: 'full', itemWidth: cardWidth, itemHeight: h, colGap: 0 },
      { items: allSlots.slice(1, 2), layoutType: 'full', itemWidth: cardWidth, itemHeight: h, colGap: 0 },
    ];
  } else if (maxSlots === 3) {
    // 3 member view: 3 stacked boxes with standard 3-box spacing
    const h = base3CardHeight;
    rowGap = 2;
  ...
  const renderSlotCard = (
    ...
    const vTop = (itemHeight - vHeight) / 2;
    const vLeft = (itemWidth - vWidth) / 2;
    ...
    return (
      ...
      >
          ...
          <Video
            ...
            style={
              (member as any)?.needsRotation !== false
                ...
                : [StyleSheet.absoluteFill, { borderRadius: cardRadius }]
            }
        ...
        {/* Center: When Video is present -> ONLY Time */}
        {hasVideo ? (
            ...
            {isCurrentUser && !showGroupDropdown && (
              ...
              >
                ...
                >
                  ...
                  <Svg width="100%" height="100%" style={StyleSheet.absoluteFillObject}>
                    <Defs>
                      <LinearGradient id="grpTapBorderGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                        <Stop offset="0%" stopColor="#FFFFFF" stopOpacity={isDark ? 0.40 : 0.80} />
                        <Stop offset="100%" stopColor={isDark ? '#FFFFFF' : '#000000'} stopOpacity={isDark ? 0.08 : 0.12} />
                      </LinearGradient>
                    </Defs>
                    <Rect
                      x={0.6}
        ...
        {/* BOTTOM-RIGHT: 3-DOTS OPTIONS MENU ICON inside respective card */}
        <TouchableOpacity
          activeOpacity={0.7}
          style={[
          ...
          onPress={() => {
            if (showOptionsMenu && selectedMemberForOptions?.id === member.id) {
        ...
        {showOptionsMenu && selectedMemberForOptions?.id === member.id && (
          ...
          >
            ...
            {/* 1. Edit Caption (Always Available) */}
            <TouchableOpacity
              style={{
                flexDirection: 'row',
                alignItems: 'center',
            ...
            {/* 2 & 3. Save and Delete (ONLY when video pal is captured) */}
            {hasVideo && (
              <>
                {/* 2. Save */}
                <TouchableOpacity
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Center: Pal Group Name Liquid Glass Capsule with Dropdown Menu Trigger */}
        <View style={[styles.centerHeaderGroup, { top: Math.max(insets.top - 1, 7) - 1.5 }]} pointerEvents="box-none">
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => setShowGroupDropdown(!showGroupDropdown)}
          >
            <LiquidGlassCapsule
              idPrefix="palGroupHeader"
              isDark={isDark}
              width={110}
              height={45}
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, zIndex: 10, paddingHorizontal: 6 }}>
                <Text
                  style={[
                    styles.vlogPillText,
                  ...
                  numberOfLines={1}
                >
                  {currentGroupName || group?.name || 'pals'}
                </Text>
                <Ionicons
          ...
          {/* Group Page Indicator Dot */}
          <View style={[styles.headerPageDot, { backgroundColor: isDark ? '#FFFFFF' : '#000000' }]} />
        </View>

        {/* Top Right: Share (Export) Button + Group Chat Button */}
        <View style={styles.headerRightCluster}>
          <LiquidGlassIconButton
            idPrefix="btnPalShare"
            isDark={isDark}
        ...
        {rowConfigs.map((row, rowIdx) => (
          <View
            ...
            style={{
              width: cardWidth,
              flexDirection: 'row',
              justifyContent: row.isCentered ? 'center' : 'space-between',
              alignItems: 'center',
              gap: row.colGap,
            }}
          >
            {row.items.map((slot) =>
              renderSlotCard(slot, row.itemWidth, row.itemHeight, row.layoutType)
            )}
          </View>
        ))}
      </ScrollView>
        ...
        >
          <TouchableOpacity
            style={{
              ...
              backgroundColor: 'transparent',
              alignItems: 'center',
              paddingTop: Math.max(insets.top - 1, 7) + 50.5,
            }}
            activeOpacity={1}
            onPress={() => {
              setShowGroupDropdown(false);
